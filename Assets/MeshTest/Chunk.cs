...
public class Chunk : MonoBehaviour {
    ...
    void Start() {
        ...
        map = new byte[World2.currentWorld.chunkWidth, World2.currentWorld.chunkHeight, World2.currentWorld.chunkWidth];

        for (int x = 0; x < World2.currentWorld.chunkWidth; x++) {
            for (int z = 0; z < World2.currentWorld.chunkWidth; z++) {
                map[x, 0, z] = 1;
    ...
    public virtual void CreateVisualMesh() {
        ...
        for (int x = 0; x < World2.currentWorld.chunkWidth; x++) {
            for (int y = 0; y < World2.currentWorld.chunkHeight; y++) {
                for (int z = 0; z < World2.currentWorld.chunkWidth; z++) {
    ...
    public virtual byte GetByte (int x, int y, int z ) {
        if((x<0)||(y<0)||(z<0)||(y>=World2.currentWorld.chunkHeight)|| (x >= World2.currentWorld.chunkWidth)|| (z >= World2.currentWorld.chunkWidth)){
            return 0;
        }
        return map[x, y, z];
    }
